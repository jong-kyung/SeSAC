...
from models import users, auth_user, admin
...
def sign_up():
    ...
    find_user = Login_query(auth_user) # 회원가입시 접근하는 DB
    crm_user = Login_query(users) # 사용자의 정보를 저장하는 DB
...
def login():
    ...
    if user_id:
        ...
        token = jwt.encode(payload, SECRET_KEY, algorithm='HS256')
        response = make_response(redirect('/user')) # 응답객체 생성
