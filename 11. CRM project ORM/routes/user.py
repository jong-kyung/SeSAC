...
def user_list():  
    ...
    try:
        ...
        user = User_query(users)
        result_datas = [] # 결과 데이터 삽입용
        datas = user.total_data_query(page, per_page, search_name, sub_data)
        # 페이지네이션
        ...
        return render_template('component/user.html', search_name = search_name, sub_data = sub_data, page = page, datas = result_datas, page_range = page_range, start_page = start_page, end_page = end_page)
...
def user_info(param):
    ...
    return render_template('component/user_detail.html', datas=findData, stores = stores)
